...
class TranslationGenerator:
    ...
    async def translate_and_write(cls, lang, data):
        try:
            ...
            with open('translations.js', 'a', encoding='utf-8') as js_file:
                js_file.write(
    ...
    def generate_translations(cls, target_languages):
        try:
            ...
            with open('data.json', 'r', encoding='utf-8') as txt_file:
                data_str = txt_file.read()

            # Parse the JSON data
            translations_data = json.loads(data_str)

            # Create a .js file and write the data with the desired structure
            with open('translations.js', 'w', encoding='utf-8') as js_file:
                js_file.write('const translations = {\n')
                js_file.write("  en: {\n")
                for key, value in translations_data['en'].items():
                    js_file.write(
                        f"    {json.dumps(key, ensure_ascii=False)}: {json.dumps(value, ensure_ascii=False)},\n")
                js_file.write("  },\n")
            ...
            async def main():
                async with aiohttp.ClientSession() as session:
                    tasks = [cls.translate_and_write(
                        lang, translations_data['en']) for lang in target_languages]
                    await asyncio.gather(*tasks)

            # Run the async event loop
            asyncio.run(main())

            # Complete the .js file
            with open('translations.js', 'a', encoding='utf-8') as js_file:
                js_file.write('};\n')
...
target_languages = ['ar', 'fr']  # Add more languages as needed
...
supported_langs = trans.get_supported_languages()
